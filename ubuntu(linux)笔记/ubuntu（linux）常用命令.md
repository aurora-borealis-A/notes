...
```bash
tac 111.txt  # 在终端上先打印文件111.txt的最后一行，然后倒数第二行，然后倒数第三行…………
```
...
```bash
head 111.txt  # 显示111.txt的前10行
head -5 111.txt  # 显示111.txt的前5行
head -15 111.txt  # 显示111.txt的前15行
```
...
```bash
tail 111.txt  # 显示111.txt的末尾10行
tail -5 111.txt  # 显示111.txt的末尾5行
tail -15 111.txt  # 显示111.txt的末尾15行
```
...
"SSH File Transfer Protocal"的缩写，安全文件传输协议，用于交互式地传输文件，在传输多个文件时比scp方便。
